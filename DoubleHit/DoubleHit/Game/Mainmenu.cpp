...
void Mainmenu::Load() {   
    ...
    space_texture = Engine::GetFont(static_cast<int>(Fonts::Basic)).PrintToTexture("Start", 0xF4F4F4FF);
...
void Mainmenu::Update([[maybe_unused]] double dt) {
    ...
    if (GetMouseX() >= Engine::GetWindow().GetSize().x / 2 - space_texture->GetSize().x / 2
        ...
        && GetMouseY() <= 480 + space_texture->GetSize().y / 2) {
        space_texture = Engine::GetFont(static_cast<int>(Fonts::Basic)).PrintToTexture("Start", 0x3b5dc9ff);
